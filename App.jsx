...
function LmsEnterprisePortal() {
    ...
    try {
      ...
      if (userFound) {
        if (userFound.status === 'archived') {
          alert("Sua conta foi inativada/arquivada pela Gestão. Acesso bloqueado.");
          await signOut(auth);
  ...
  const handleToggleArchiveUser = (id) => {
    if (id === 'admin1' || id === currentUser.id) return alert('Você não pode alterar o status a si mesmo ou ao Administrador Principal.');
  ...
  const handleCloneCourse = (courseToClone) => {
    const newCode = prompt("Novo Código para a Cópia:", courseToClone.codigo + " - CÓPIA");
    if (!newCode) return;
    const newName = prompt("Novo Nome da Disciplina:", courseToClone.nome);
    if (!newName) return;

    const newCourseId = `c_${Date.now()}`;
    const newCourseObj = { ...courseToClone, id: newCourseId, codigo: newCode, nome: newName, status: 'active' };
    ...
    oldContents.forEach(sec => {
      ...
      (sec.items || []).forEach(item => {
        ...
        if (item.type === 'MessageSquare') {
          ...
          newExams[newItemId] = { questions: [...oldExam.questions], submissions: {} }; 
        }
    ...
    alert("Disciplina clonada com sucesso! Uma nova turma limpa de alunos foi gerada.");
  };
  ...
  const handleCreateAgendaTask = (e) => {
    ...
    const newTask = {
      ...
      courseId: newAgendaCourseId || null,
      createdAt: new Date().toISOString()
    };
  ...
  const handleDeleteAgendaTask = (id) => {
    if(window.confirm('Remover evento da agenda?')) {
  ...
  const visibleNotices = noticesData.filter(notice => {
    if (notice.courseId === null) return true; // Avisos globais institucionais
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-fade-in flex flex-col max-h-[90vh]">
          ...
          <div className="bg-gradient-to-r from-teal-800 to-teal-600 p-8 text-white relative flex flex-col items-center shrink-0 shadow-sm">
            ...
            {profileUser.status === 'archived' && (
              <span className="bg-red-900/50 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest mt-2 border border-red-500/30 text-red-100">
                Inativo / Egresso
              </span>
            )}
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800 tracking-tight">Agenda Acadêmica e Pessoal</h2>
            <p className="text-sm text-slate-500">Controle de tarefas, prazos e lembretes da plataforma.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit">
            <h3 className="font-bold text-slate-800 mb-5 flex items-center gap-2 text-base">
              <Plus className="w-5 h-5 text-amber-600"/> Novo Lembrete
            </h3>
            <form onSubmit={handleCreateAgendaTask} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase block mb-1.5">Tarefa</label>
                <input type="text" required value={newAgendaTitle} onChange={e => setNewAgendaTitle(e.target.value)} placeholder="Ex: Estudar anatomia" className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-slate-50 focus:bg-white" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase block mb-1.5">Data</label>
                <input type="date" required value={newAgendaDate} onChange={e => setNewAgendaDate(e.target.value)} className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-slate-50 focus:bg-white" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase block mb-1.5">Visibilidade</label>
                <select value={newAgendaCourseId} onChange={e => setNewAgendaCourseId(e.target.value)} className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-slate-50 focus:bg-white cursor-pointer">
                  <option value="">Lembrete Pessoal</option>
                  {(role === 'admin' || role === 'professor') && visibleCourses.map(c => (
                    <option key={c.id} value={c.id}>Para a turma: {c.codigo}</option>
                  ))}
                </select>
              </div>
              <button type="submit" className="w-full bg-amber-600 text-white font-semibold p-3 rounded-lg hover:bg-amber-700 transition shadow-sm text-sm flex items-center justify-center gap-2 mt-2">
                <Check className="w-4 h-4"/> Salvar na Agenda
              </button>
            </form>
          </div>
          ...
          <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="font-bold text-slate-800 mb-5 flex items-center gap-2 text-base">
              <Calendar className="w-5 h-5 text-slate-600"/> Próximos Compromissos
            </h3>
            <div className="space-y-3">
              {visibleAgenda.length === 0 ? (
                ...
                visibleAgenda.map(task => {
                  ...
                  const courseName = isPersonal ? 'Pessoal' : courses.find(c => c.id === task.courseId)?.codigo || 'Turma Oculta';
                  const canDelete = task.creatorId === currentUser.id || role === 'admin';
                  return (
                    <div key={task.id} className="p-4 border border-slate-200 rounded-xl hover:shadow-sm transition-all bg-white flex items-center justify-between gap-4 group">
                      <div className="flex gap-4 items-center">
                        <div className="w-12 h-12 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center font-bold text-lg border border-amber-100 shrink-0">
                          {new Date(task.date).getDate() + 1}
                        </div>
                        <div>
                          <h4 className="font-bold text-slate-800 text-base mb-1">{task.title}</h4>
                          <div className="flex items-center gap-2">
                            <span className={`text-[9px] font-bold px-2 py-0.5 rounded uppercase tracking-wider border ${isPersonal ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                              {courseName}
                            </span>
                            <span className="text-xs text-slate-400 font-medium">{new Date(task.date).toLocaleDateString()}</span>
                          </div>
                        </div>
                      </div>
                      {canDelete && (
                        <button onClick={() => handleDeleteAgendaTask(task.id)} className="text-slate-300 hover:text-emerald-500 p-2 rounded-lg transition-all opacity-0 group-hover:opacity-100" title="Concluir">
      ...
      {visibleNotices.length > 0 && (
        <div className="bg-amber-50 p-6 rounded-2xl border border-amber-200 shadow-sm">
          <h3 className="font-bold text-amber-900 mb-4 flex items-center gap-2 text-lg">
            <Megaphone className="w-5 h-5 text-amber-600"/> Mural de Avisos
          </h3>
          <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
            {visibleNotices.map(notice => (
              <div key={notice.id} className="min-w-[280px] w-[320px] bg-white p-4 rounded-xl border border-amber-100 shadow-sm shrink-0 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded uppercase">
                      {notice.courseId ? 'Aviso da Turma' : 'Aviso Institucional'}
                    </span>
                    {(role === 'admin' || notice.authorId === currentUser.id) && (
                      <button onClick={() => handleDeleteNotice(notice.id)} className="text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4"/></button>
                    )}
                  </div>
                  <h4 className="font-bold text-slate-800 text-base mb-1">{notice.title}</h4>
  ...
  const renderAdminDashboard = () => (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Quadro de Disparo de Avisos Globais (Admin) */}
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-6">
        <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2 text-base">
          <Megaphone className="w-5 h-5 text-amber-600"/> Disparar Aviso Institucional
        </h3>
        <form onSubmit={handleCreateNotice} className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 space-y-3">
            <input type="text" required value={newNoticeTitle} onChange={e => setNewNoticeTitle(e.target.value)} placeholder="Título (Ex: Feriado Nacional)" className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-slate-50 focus:bg-white" />
            <textarea required value={newNoticeContent} onChange={e => setNewNoticeContent(e.target.value)} placeholder="Corpo da mensagem para toda a instituição..." className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-slate-50 focus:bg-white h-12 resize-none"></textarea>
          </div>
          <button type="submit" className="bg-amber-600 text-white font-semibold px-6 rounded-lg hover:bg-amber-700 transition shadow-sm shrink-0 flex items-center justify-center gap-2 text-sm">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-center mb-5">
            ...
            <button 
              ...
              className={`px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-colors border ${showArchivedCourses ? 'bg-slate-800 text-white border-slate-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}
            >
          ...
          {(showArchivedCourses ? courses : activeCoursesGlobally).length === 0 ? (
            <div className="text-center p-10 border border-dashed border-slate-300 rounded-xl bg-slate-50 text-slate-500 text-sm">
              Nenhuma disciplina encontrada nos filtros atuais.
            </div>
          ) : (
            <div className="space-y-3">
              {(showArchivedCourses ? courses : activeCoursesGlobally).map(c => {
                ...
                return (
                  <div 
                    key={c.id} 
                    className={`p-4 border border-slate-200 rounded-xl transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 group ${isArchived ? 'bg-slate-50 opacity-70' : 'bg-white hover:border-blue-300 hover:shadow-sm'}`}
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {isArchived && <span className="text-[10px] font-bold text-amber-800 bg-amber-100 px-2 py-0.5 rounded uppercase tracking-wider">Arquivada</span>}
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {!isArchived && (
                        <>
                          ...
                          <button 
                            ...
                            className="bg-white border border-slate-200 text-slate-600 px-3 py-2 rounded-lg text-xs hover:bg-amber-50 hover:text-amber-600 transition"
                            title="Clonar disciplina (Criar turma nova com mesmo conteúdo)"
                          >
                      ...
                      <button 
                        ...
                        className={`px-3 py-2 rounded-lg text-xs transition border ${isArchived ? 'bg-emerald-50 border-emerald-100 text-emerald-600 hover:bg-emerald-100' : 'bg-white border-slate-200 text-slate-400 hover:bg-slate-100'}`}
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              ...
              className={`px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-colors border ${showArchivedUsers ? 'bg-slate-800 text-white border-slate-900' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-200'}`}
            >
              <Archive className="w-3.5 h-3.5 inline mr-1.5"/>
              {showArchivedUsers ? 'Ocultar Inativos' : 'Ver Inativos'}
            </button>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {Object.values(systemUsers)
                  ...
                  return (
                    <tr key={u.id} className={`transition-colors group ${isArchived ? 'bg-slate-50 opacity-70' : 'hover:bg-slate-50'}`}>
                          ...
                          {isEditingThisUser ? (
                            ...
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2">
                                ...
                                {isArchived && <span className="bg-red-100 text-red-700 border border-red-200 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase">Inativo</span>}
                              </div>
                      ...
                      <td className="p-4 text-right whitespace-nowrap">
                        {isEditingThisUser ? (
                          ...
                          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            {!isArchived && <button onClick={() => handleStartEditUser(u)} className="text-slate-400 hover:text-purple-600 bg-slate-50 hover:bg-purple-50 p-2 rounded transition border border-transparent hover:border-purple-200" title="Editar"><Edit2 className="w-4 h-4"/></button>}
                            {u.id !== 'admin1' && u.id !== currentUser.id && (
                              <button onClick={() => handleToggleArchiveUser(u.id)} className={`p-2 rounded transition border border-transparent ${isArchived ? 'text-emerald-500 hover:text-emerald-700 hover:bg-emerald-50 hover:border-emerald-200' : 'text-slate-400 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-200'}`} title={isArchived ? 'Reativar Usuário' : 'Arquivar Usuário'}>
                                <Archive className="w-4 h-4"/>
                              </button>
                            )}
                          </div>
  ...
  const renderCourseHome = () => (
    <div className="animate-fade-in relative">
      ...
      {isEditing && (
        <div className="bg-amber-50 border border-amber-200 p-5 rounded-2xl mb-6 shadow-sm print:hidden">
          <h3 className="font-bold text-amber-900 mb-3 flex items-center gap-2 text-base">
            <Megaphone className="w-5 h-5 text-amber-600"/> Enviar Aviso para esta Turma
          </h3>
          <form onSubmit={handleCreateNotice} className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 space-y-3">
              <input type="text" required value={newNoticeTitle} onChange={e => setNewNoticeTitle(e.target.value)} placeholder="Título do aviso..." className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-white" />
              <textarea required value={newNoticeContent} onChange={e => setNewNoticeContent(e.target.value)} placeholder="Mensagem para os residentes desta turma..." className="w-full border border-slate-300 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 outline-none bg-white h-12 resize-none"></textarea>
            </div>
            <button type="submit" className="bg-amber-600 text-white font-semibold px-6 rounded-lg hover:bg-amber-700 transition shadow-sm shrink-0 flex items-center gap-2 text-sm">
              <Send className="w-4 h-4"/> Enviar
            </button>
          </form>
        </div>
                          ...
                          <div className="mt-3 flex flex-wrap gap-3 print:hidden">
                            ...
                            {item.type === 'NativeExam' && (
                              <button onClick={() => { setActiveExamItem(item); setLocalQuestions(exams[item.id]?.questions || []); }} className="flex items-center gap-1.5 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 px-4 py-2 rounded-lg transition shadow-md w-fit">
                                <ClipboardList className="w-4 h-4"/> {(role === 'professor' || role === 'admin') ? 'Configurar Prova / Ver Notas' : 'Fazer Avaliação'}
                              </button>
                            )}
        ...
        <div className="flex-1 overflow-y-auto py-6 no-scrollbar relative">
          <nav className="space-y-2 px-4">
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-lg text-sm font-semibold transition-all mt-1 ${currentView === 'agenda' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
              <Calendar className="w-5 h-5 flex-shrink-0" />
              {sidebarOpen && <span>Agenda do Aluno</span>}
            </button>
