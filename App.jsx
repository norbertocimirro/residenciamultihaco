...
function LmsEnterprisePortal() {
  ...
  const [dbUsers, setDbUsers, usersLoaded] = useFirestoreDB('tb_users', {
    ...
    'stu1': { id: 'stu1', nome: 'Mariana Alves', role: 'aluno', avatar: 'MA', email: 'mariana.aluno@teste.com', phone: '(51) 98888-8888', bio: 'Residente do primeiro ano.', showContactPublicly: false }
  });
  ...
  const updateItemTitle = (sectionId, itemId, newTitle) => updateContent(activeContent.map(sec => { if (sec?.id === sectionId) { return { ...sec, items: (sec.items || []).map(item => item?.id === itemId ? { ...item, title: newTitle } : item) }; } return sec; }));
  const deleteItem = (sectionId, itemId) => { if (window.confirm('Apagar este material?')) updateContent(activeContent.map(sec => { if (sec?.id === sectionId) { return { ...sec, items: (sec.items || []).filter(item => item?.id !== itemId) }; } return sec; })); };
  ...
  const renderProfileModal = () => {
    ...
    const canSeePrivate = isMe || role === 'admin' || (role === 'professor' && profileUser.role === 'aluno') || profileUser.showContactPublicly;
          ...
          <div className="p-8 overflow-y-auto flex-1 bg-slate-50">
            {editProfileData ? (
              <form onSubmit={saveProfile} className="space-y-6">
                ...
                <div className="bg-teal-50 border border-teal-200 p-4 rounded-xl flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-xs text-teal-700">Se ativo, alunos verão seu email e telefone.</span>
                  </div>
              ...
              <div className="space-y-8">
                <div>
                  <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2"><Info className="w-4 h-4"/> Sobre</h3>
                  <p className="text-slate-700 text-base leading-relaxed whitespace-pre-wrap bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                    {profileUser.bio || <span className="italic text-slate-400">Nenhuma biografia fornecida.</span>}
                  </p>
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex items-center gap-4">
                        ...
                        <div className="overflow-hidden">
                          ...
                          <span className="font-medium text-slate-800 truncate block">{profileUser.email || '-'}</span>
                        </div>
                      </div>
                      <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex items-center gap-4">
                        ...
                        <div className="overflow-hidden">
                          ...
                          <span className="font-medium text-slate-800 truncate block">{profileUser.phone || '-'}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                  ...
                  {forumData.length === 0 ? (
                    ...
                    forumData.map(topic => (
                      <div key={topic.id} onClick={() => setActiveTopicId(topic.id)} className="bg-white p-5 border border-slate-200 rounded-xl hover:border-purple-300 hover:shadow-md cursor-pointer transition flex items-center gap-5">
                        <div onClick={(e) => {e.stopPropagation(); setViewingProfileId(topic.authorId)}} className="w-12 h-12 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center text-sm font-black shrink-0 border border-slate-200 hover:border-purple-500 transition">{topic.avatar}</div>
                        <div className="flex-1">
                          <h4 className="font-bold text-slate-800 text-lg group-hover:text-purple-700 transition">{topic.title}</h4>
                          <p className="text-xs text-slate-500 mt-1">Iniciado por <strong className="text-slate-700 hover:text-purple-700" onClick={(e) => {e.stopPropagation(); setViewingProfileId(topic.authorId)}}>{topic.authorName}</strong> em {new Date(topic.createdAt).toLocaleString()}</p>
                        </div>
                    ...
                    {(currentTopicData?.replies || []).map((reply, idx) => (
                      <div key={reply.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative">
                        ...
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-3">
                            <div onClick={() => setViewingProfileId(reply.authorId)} className="cursor-pointer w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-xs font-bold border border-slate-200 hover:border-purple-500">{reply.avatar}</div>
                            <div>
                              <p onClick={() => setViewingProfileId(reply.authorId)} className="font-bold text-slate-800 text-xs cursor-pointer hover:text-purple-700">{reply.authorName}</p>
  ...
  const renderExamModal = () => {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(examData.submissions).map(([studentId, sub]) => (
                      <div key={studentId} className="flex justify-between items-center bg-white border border-slate-200 p-4 rounded-xl shadow-sm">
                        <div className="flex items-center gap-3">
                           <div onClick={() => setViewingProfileId(studentId)} className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-xs font-bold border border-slate-200 cursor-pointer hover:border-teal-500">{systemUsers[studentId]?.avatar || '?'}</div>
                           <div>
    ...
    const handleSumbitExam = (e) => {
      ...
      const updatedSubmissions = { 
        ...
        [currentUser.id]: { score: finalScore, answers: answers, submittedAt: new Date().toISOString() } 
      };
                ...
                return (
                  <div key={c.id} className="p-4 border border-slate-200 rounded-xl hover:shadow-md transition bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">Professor Responsável: <strong className="text-slate-700 hover:text-teal-700 cursor-pointer" onClick={() => setViewingProfileId(c.professorId)}>{systemUsers[c.professorId]?.nome || 'Não definido'}</strong></p>
